# Reefy App Specification v0.2

An Reefy App is a packaged application that runs on Reefy bare metal infrastructure. This document defines the specification for developing, packaging, and deploying apps within the Reefy ecosystem.
...
Reefy enforces a strict separation between **code** (container images) and **data** (volumes).
...
- **Only declared volumes survive.** The platform only knows about volumes declared in `reefy_myapp_volumes`. Data written to any other location — inside the container or on the host filesystem — is ephemeral and will not survive container recreation, app updates, or machine replacement.
...
An Reefy App is an Ansible role hosted in a Git repository. The platform clones the repository and runs the app's entrypoint playbook against the target VM.
...
```
git_url: https://github.com/reefyai/reefy.git
git_ref: main
playbook_path: collections/ansible_collections/reefy/compute/playbooks/run-openclaw.yml
```
...
- name: Deploy My App
  ...
  roles:
    - role: reefy.compute.docker_vm    # Docker setup (required)
    - role: reefy.compute.nvidia       # GPU drivers (if app needs GPU)
    - role: reefy.compute.myapp
```
...
Apps accept inputs through Ansible variables defined in `defaults/main.yml`. All input variables must be prefixed with `reefy_{appname}_` to avoid conflicts.
...
---
# Container image
reefy_myapp_image: org/myapp:latest

# App-specific settings
reefy_myapp_port: 8080
```
...
- name: Deploy My App
  ...
  roles:
    - role: reefy.compute.docker_vm
    - role: reefy.compute.myapp
      vars:
        reefy_myapp_api_key: "{MYAPP_API_KEY}"
...
reefy_myapp_local_config: ""

# Whether to overwrite existing config
reefy_myapp_config_force: false
```
...
- name: Copy user-provided config
  ansible.builtin.copy:
    src: "{{ reefy_myapp_local_config }}"
    dest: "{{ reefy_myapp_config_volume }}/config.yaml"
    mode: '0644'
    force: "{{ reefy_myapp_config_force }}"
  when: reefy_myapp_local_config | length > 0
...
Apps can produce any number of outputs by appending to the `reefy_outputs` list. Each output has a `show_to_user` flag controlling whether it appears in the console web UI. Apps can emit outputs at any point during execution — they are not limited to a single predefined block.
...
- name: Report app URL
  ansible.builtin.set_fact:
    reefy_outputs: "{{ reefy_outputs | default([]) + [item] }}"
  loop:
    - name: web_ui_url
...
All Reefy Apps run as Docker containers.
...
- name: Start app container
  community.docker.docker_container:
    name: "{{ reefy_myapp_container_name }}"
    image: "{{ reefy_myapp_image }}"
    pull: true
    restart_policy: unless-stopped
    ports:
      - "{{ reefy_myapp_port }}:{{ reefy_myapp_port }}"
    env: "{{ reefy_myapp_env | default({}) }}"
...
Volume mounts are constructed by the platform from `reefy_myapp_volumes` and injected automatically. The app role may also specify additional volumes directly if needed.

### GPU Container (NVIDIA)

For apps requiring GPU acceleration, include the `reefy.compute.nvidia` role in the playbook and use the NVIDIA runtime:
    ...
    device_requests:
      - driver: nvidia
        ...
        capabilities:
          - - gpu
            - nvidia
    shm_size: "{{ reefy_myapp_shm_size | default('512M') }}"
    restart_policy: unless-stopped
```
...
- name: Wait for app to be healthy
  ansible.builtin.uri:
    url: "http://localhost:{{ reefy_myapp_port }}/health"
    method: GET
...
- name: Check if config exists (may have been restored from backup)
  ansible.builtin.stat:
    path: "{{ reefy_myapp_config_volume }}/config.yaml"
...
| Variables | `reefy_{appname}_*` | `reefy_frigate_port` |
| Container name | `{appname}` (default) | `frigate` |
| Volume names | lowercase, descriptive | `config`, `data`, `media` |

## Required Variables

Every Reefy App must define these in `defaults/main.yml`:

```yaml
# Required by spec
reefy_myapp_image: org/myapp:latest     # Container image
reefy_myapp_volumes: []                 # Volume declarations (see Volumes section)
```
...
A minimal Reefy App that runs a web service:
...
- name: Set app outputs
  ansible.builtin.set_fact:
    reefy_outputs: "{{ reefy_outputs | default([]) + outputs }}"
  vars:
    outputs:
      - name: web_ui_url
        value: "https://{{ tailscale_hostname }}:8443/"
