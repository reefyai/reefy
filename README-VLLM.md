...
# Run vLLM with Nvidia GPU under Reefy Linux in Automated Way
...
ansible-playbook -i $VM_HOST, \
  ...
  -e reefy_vllm_model="Qwen/Qwen3-0.6B"
```
...
ansible-playbook -i $VM_HOST, \
  collections/ansible_collections/reefy/compute/playbooks/run-vllm.yml \
  -e reefy_vllm_model="meta-llama/Llama-3.1-8B-Instruct" \
  -e reefy_vllm_hf_token="hf_xxxxx"
```
...
ansible-playbook -i $VM_HOST, \
  collections/ansible_collections/reefy/compute/playbooks/run-vllm.yml \
  -e '{"reefy_vllm_model": "TinyLlama/TinyLlama-1.1B-Chat-v1.0", "reefy_vllm_extra_args": "--tensor-parallel-size 2 --max-model-len 2048"}'
```
