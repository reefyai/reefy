...
# Run SGLang with Nvidia GPU under Reefy Linux in Automated Way
...
![Reefy Linux: SGLang - Monitoring GPU Load, Memory, Temp, FAN speed, Power consumption (Watt) with Grafana](images/sglang-benchmark-all-gpu.png)
...
```sh
ansible-playbook -i $VM_HOST, \
  collections/ansible_collections/reefy/compute/playbooks/run-sglang.yml \
  -e reefy_sglang_model="Qwen/Qwen3-0.6B"
```
...
ansible-playbook -i $VM_HOST, \
  ...
  -e reefy_sglang_hf_token="hf_xxxxx"
```
...
ansible-playbook -i $VM_HOST, \
  collections/ansible_collections/reefy/compute/playbooks/run-sglang.yml \
  -e '{"reefy_sglang_model": "TinyLlama/TinyLlama-1.1B-Chat-v1.0", "reefy_sglang_extra_args": "--dp 2"}'
```
...
```bash
ansible-playbook -i $VM_HOST, \
  collections/ansible_collections/reefy/compute/playbooks/run-genai-perf.yml \
  -e reefy_genai_perf_model="Qwen/Qwen3-0.6B" \
  -e reefy_genai_perf_concurrency=24
```
...
- A GPU-enabled VM on Bare Metal running Reefy Linux
