#!/bin/bash
#
# Send Ansible Playbook via MQTT
#
# Packages an ansible playbook (file or directory) as a base64-encoded
# tar.gz bundle and publishes it to a device's command topic over MQTT.
# The device decodes, extracts, and runs ansible-playbook locally.
#
# Usage:
#   ./send-playbook.sh -c CERTS_DIR -d UUID PLAYBOOK_PATH
#
# Options:
...
#
# Examples:
#   # Send a single playbook file
#   ./send-playbook.sh -c ./device-certs/UUID -d UUID playbook.yml
#
#   # Send a playbook directory with roles
#   ./send-playbook.sh -c ./device-certs/UUID -d UUID ./my-playbook/
#
#   # Send and watch for result
#   ./send-playbook.sh -c ./device-certs/UUID -d UUID -w playbook.yml
#
...
if [[ -z "${PLAYBOOK_PATH}" ]]; then
    log_error "Playbook path required (positional argument)"
    exit 1
fi
...
log_info "Packaging playbook: ${PLAYBOOK_PATH}"

if [[ -f "${PLAYBOOK_PATH}" ]]; then
    # Single file - tar it up
    BASENAME=$(basename "${PLAYBOOK_PATH}")
    cp "${PLAYBOOK_PATH}" "${TMPDIR}/${BASENAME}"
    tar czf "${TMPDIR}/bundle.tar.gz" -C "${TMPDIR}" "${BASENAME}"
elif [[ -d "${PLAYBOOK_PATH}" ]]; then
    # Directory - tar the contents
    tar czf "${TMPDIR}/bundle.tar.gz" -C "${PLAYBOOK_PATH}" .
fi
...
if [[ ${B64_SIZE} -gt 900000 ]]; then
    ...
    log_error "Consider using apply_config with bundle_url instead"
    exit 1
fi

# Build JSON payload
PAYLOAD=$(jq -n \
    --arg action "run_playbook" \
    --arg playbook_b64 "${BUNDLE_B64}" \
    --arg version "${VERSION}" \
    '{
        action: $action,
...
if mosquitto_pub "${MQTT_ARGS[@]}" \
    ...
    -m "${PAYLOAD}" \
    -q 1 2>/dev/null; then
    log_info "Playbook sent successfully"
else
    log_error "Failed to publish playbook"
    exit 1
fi
