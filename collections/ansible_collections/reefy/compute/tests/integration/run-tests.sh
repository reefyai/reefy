...
LOG_FILE="/tmp/reefy-integration-test-${TIMESTAMP}.log"
...
PLAYBOOKS_DIR="$PROJECT_ROOT/collections/ansible_collections/reefy/compute/playbooks"

# All possible VM names (each service gets its own VM)
ALL_VMS=(
  "reefy-test-cpu-${TEST_SUFFIX}"
  "reefy-test-gpu-${TEST_SUFFIX}"
  "reefy-test-fryer-${TEST_SUFFIX}"
  "reefy-test-vllm-${TEST_SUFFIX}"
  "reefy-test-sglang-${TEST_SUFFIX}"
  "reefy-test-frigate-${TEST_SUFFIX}"
  "reefy-test-ollama-${TEST_SUFFIX}"
  "reefy-test-openclaw-${TEST_SUFFIX}"
)
...
cleanup_on_interrupt() {
  ...
  for vm in "${ALL_VMS[@]}"; do
    ...
    ansible-playbook -i "$HOST," "$PLAYBOOKS_DIR/stop-vm.yml" \
      ...
      -e reefy_vm_persist_boot_image=false 2>/dev/null || true
  done
  echo "  Cleanup done."
  exit 130
}
...
echo "=========================================="
echo "  Reefy Integration Test Suite"
echo "=========================================="
echo "  Host:          $HOST"
echo "  Suffix:        $TEST_SUFFIX"
echo "  Skip GPU:      $SKIP_GPU"
echo "  Log file:      $LOG_FILE"
echo "  VMs:           reefy-test-{cpu,gpu,fryer,vllm,sglang,frigate,ollama,openclaw}-${TEST_SUFFIX}"
echo "=========================================="
echo ""
