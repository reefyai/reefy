...
---
module: qemu_vm
short_description: Manage Reefy QEMU virtual machines with SVSM support
version_added: "1.0.0"
description:
...
options:
  ...
  tailscale_tags:
    description:
      ...
      - Comma-separated list of tags (e.g., "tag:reefy,tag:dev")
...
image_path:
  ...
  returned: when state is present/started
  type: str
  sample: "/mnt/reefy-data/images/dev-vm-01/dev-vm-01.qcow2"
...
class QemuVm:
    ...
    def start_container(self, qemu_cmd):
        ...
        if use_standard:
            ...
            container_image = 'reefy/qemu-standard'
            full_cmd = qemu_cmd
        else:
    ...
    def build_qemu_command(self, gpus, data_disk_path):
        ...
        if data_disk_path:
            cmd_parts.extend([
                ...
                '-device', 'scsi-hd,drive=datadisk0,bus=scsi0.0,lun=1,serial=reefy-data-disk',
            ])
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            storage_path=dict(type='path', default='/mnt/reefy-data'),
            bridge=dict(type='str', default='br0'),
            container_image=dict(type='str', default='reefy/svsm'),
            persist_boot_image=dict(type='bool', default=True),
            root_password=dict(type='str', no_log=True),
            tailscale_tags=dict(type='str', default='tag:reefy'),
