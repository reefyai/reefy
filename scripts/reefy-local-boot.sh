#!/bin/bash
#
# Boot Reefy Linux in QEMU for local testing
#
...
#   reefy-local-boot.sh -d data.qcow2          # Attach data disk
#   reefy-local-boot.sh -- -serial mon:stdio   # Custom QEMU args
...
if $READONLY; then
    ...
    IMG_FILE="${IMG_DIR}/reefy-qemu.raw"
