...
In this tutorial, we will run `"Qwen/Qwen2.5-VL-3B-Instruct"` in an automated way using the **Reefy Linux** distro and **Ansible**.
...
ansible-playbook -i $VM_HOST, \
  ...
  -e reefy_sglang_model="Qwen/Qwen2.5-VL-3B-Instruct" \
  -e 'reefy_sglang_extra_args="--dp 2 --disable-radix-cache --chunked-prefill-size -1 --chat-template qwen2-vl --mem-fraction-static 0.7"'
```
...
```bash
LLM_URL="http://reefy-vm-67f97659333f:8000/v1/chat/completions"
...
You're now running Qwen2.5-VL using either vLLM or SGLang on your own Reefy Linux VM with full GPU acceleration.
